  ...
  methods: {
    ...
    async saveProfile () {
      ...
      try {
        ...
        if (resp.succeeded) {
          Cookies.set('jwt', _.get(respRaw, 'data.users.updateProfile.jwt', ''), { expires: this.$clientConfig.getJwtExpires() })
      ...
      if (validation) {
        ...
        try {
          ...
          if (resp.succeeded) {
            ...
            this.newPass = ''
            this.verifyPass = ''
            Cookies.set('jwt', _.get(respRaw, 'data.users.changePassword.jwt', ''), { expires: this.$clientConfig.getJwtExpires() })
            this.$store.commit('showNotification', {
