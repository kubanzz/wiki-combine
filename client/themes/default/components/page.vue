<template lang="pug">
  v-app(v-scroll='upBtnScroll', :dark='$vuetify.theme.dark', :class='$vuetify.rtl ? `is-rtl` : `is-ltr`')
    ...
    v-main(ref='content' v-if='path !== `home`')
        ...
        v-layout(row)
          ...
          v-flex.page-col-content(
            ...
            .comments-container#discussion(v-if='commentsEnabled && commentsPerms.read && !printView')
              ...
              .comments-main
                slot(name='comments')
    v-main(ref='homeContent' v-if='path === `home`')
        v-card.radius-7.animated.fadeInUp.wait-p2s
          v-toolbar(:color='$vuetify.theme.dark ? `grey darken-2` : `grey lighten-5`', dense, flat)
            v-spacer
            .h6 最新发布内容
            v-spacer
        v-card.radius-7.animated.fadeInUp.wait-p2s(elevation=6 )
          v-data-table.pb-2.ml-5(
            :items='filteredPages'
            :headers='headers'
            ...
            template(slot='item', slot-scope='props')
              tr.is-clickable(:active='props.selected', @click='articleRowClick(props.item)')
                td
          ...
          v-toolbar(:color='$vuetify.theme.dark ? `grey darken-2` : `grey lighten-5`', dense, flat)
            v-spacer
            .overline 找到 {{articleNum}} 条最近发布内容。
            v-spacer
...
import Vue from 'vue'
import pagesQuery from 'gql/common/common-homepages-query-list.gql'
...
export default {
  ...
  apollo: {
    pages: {
      ...
      update(data) {
        return data.pages.list2
      },
      watchLoading (isLoading) {
        this.loading = isLoading
        this.$store.commit(`loading${isLoading ? 'Start' : 'Stop'}`, 'page')
      },
      variables () {
        return {
