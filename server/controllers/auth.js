...
router.all('/login/:strategy/callback', async (req, res, next) => {
  ...
  try {
    ...
    res.cookie('jwt', authResult.jwt, { expires: moment().add(WIKI.config.jwt.amount, WIKI.config.jwt.unit).toDate() })
...
router.get('/verify/:token', bruteforce.prevent, async (req, res, next) => {
  try {
    ...
    if (WIKI.config.auth.enforce2FA) {
      ...
      res.cookie('jwt', result.token, { expires: moment().add(WIKI.config.jwt.amount, WIKI.config.jwt.unit).toDate() })
      res.redirect('/')
    }
  } catch (err) {
    next(err)
