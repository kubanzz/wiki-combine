...
module.exports = {
  ...
  PageQuery: {
    ...
    /**
     * home page LIST PAGES
     */
    async list2(obj, args, context, info) {
      let results = await WIKI.models.pages.query().column([
        'pages.id',
        'path',
        { locale: 'localeCode' },
      ...
      if (args.limit) {
        results = results.splice(0, args.limit)
      }
      if (args.tags && args.tags.length > 0) {
        results = _.filter(results, r => _.every(args.tags, t => _.includes(r.tags, t)))
      }
    ...
    async tree(obj, args, context, info) {
      WIKI.logger.info('路径：' + args.path + '开始执行-----tree方法----')
      let curPage = null
    ...
    async buildTree(obj, args, context, info) {
      if (!args.locale) { args.locale = WIKI.config.lang.code }

      if (!args.path) {
        return []
      }
      let curPage = await WIKI.models.knex('pageTree').first('ancestors').where({
        path: args.path,
        localeCode: args.locale
      })
      if (!curPage || !curPage.ancestors || curPage.ancestors.length < 1) {
        return []
      }
      ...
      let parentPage = await WIKI.models.knex('pageTree').where(builder => {
        ...
        builder.whereIn('id', ancestors)
      })

      let childrens = await WIKI.models.knex('pageTree').where(builder => {
        builder.where('localeCode', args.locale)
        builder.whereIn('parent', ancestors)
      })
  ...
  PageMutation: {
    ...
    /**
     *  ADD PAGE TREE(ADD FOLDER)
     */
    async addPageTree(obj, args, context) {
      try {
        let page = {
          path: args.path,
          localeCode: args.locale
